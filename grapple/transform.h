                             ...
                             Predicate pred)
{
    ...
    ForwardIterator ret = thrust::transform_if(exec.policy(get_system(first,result)), first, last, result, op, pred);
    exec.stop();
...
{
    exec.start(THRUST_TRANSFORM_IF);
    ForwardIterator ret = thrust::transform_if(exec.policy(get_system(first,stencil,result)), first, last, stencil, result, op, pred);
    exec.stop();
...
{
    exec.start(THRUST_TRANSFORM_IF);
    ForwardIterator ret = thrust::transform_if(exec.policy(get_system(first1,first2,stencil,result)), first1, last1, first2, stencil, result, binary_op, pred);
    exec.stop();
